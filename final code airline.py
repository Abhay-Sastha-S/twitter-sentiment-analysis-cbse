...
raw_data = pd.read_csv("C:/Users/MSI/Desktop/school/IP project/final final data.csv")
# print(raw_data)
...
positive_words = []
with open("C:/Users/MSI/Desktop/school/IP project/crazy airline/positive_words.txt") as pos_words_list:
    for lin in pos_words_list:
        if lin[0] != ';' and lin[0] != '\n':
            positive_words.append(lin.strip())
negative_words = []
with open("C:/Users/MSI/Desktop/school/IP project/crazy airline/negative_words.txt") as neg_words_list:
...
def search_in_data_file(searchdatafile, search_word=input('enter search term --')):
    ...
    print("\n", "loading..... may take upto a few minutes")
    ...
    net_score_list = []
    net_score_list = fltrd_df.iloc[:, 11]
    ...
    plt.bar(names, height, width=0.8, color=['#003366', '#6F9FE6', '#66FFFF'])
    plt.title('Tweet sentiments')
    plt.show()

    # mapping the location of tweets
    coord_df = pd.DataFrame()
    select_col = fltrd_df[["longitudes","latitudes","text"]]
    coord_df = select_col.copy()
    #print("this cord", coord_df)
    # coord_df["details"] = [fltrd_df.iloc[:,2]]
    coord_df["color coding"] = "#17BECF"
    fig = px.scatter_geo(coord_df, lat='latitudes', lon='longitudes', hover_name='text',
                         color="color coding", projection="natural earth")
