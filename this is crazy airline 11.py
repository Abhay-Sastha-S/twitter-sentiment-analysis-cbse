...
# data_file_raw = input('FULL PATH ADDRESS FOR FILE TO RAW DATASET --')
# filtered_file = input('FULL PATH ADDRESS FOR FILE TO SAVE FILTERED DATASET --')
# result_file = input('FULL PATH ADDRESS FOR FILE TO SAVE RESULTS --')

# project_data_file_raw = open(data_file_raw)
# results_data_file = open(result_file, "w+")
# project_data_file = open(filtered_file, 'r+')
...
def write_in_data_file(resultingdatafile, pos_count=0, neg_count=0, neut_count=0, tweet_count=0):
    ...
    #print("am i the culprit?", lines_project_data_file)
    # header_dont_used = lines_project_data_file.pop(0)
    # print(header_dont_used)
    # print("\n", "Sample Data of Tweets", '\n', lines_project_data_file, "\n")
    for line_read in project_data_file:
        print("this here -",line_read)
        lines_project_data_file.append(line_read)

    print("am i the culprit?", lines_project_data_file)
    for lines_tweet_data in lines_project_data_file:

        print("hello", lines_tweet_data)
        list_tweet_data = lines_tweet_data.strip().split(',')
        print("bye", list_tweet_data)
        ...
        if net_score > 0:
            ...
            x = "neutral"
            neut_count += 1
        print(pos_count, neg_count, net_score)
        resultingdatafile.write(
...
def search_in_data_file(searchdatafile, search_word=input('enter search term --')):  # search_condition = 0):
    ...
    for lines_tweet_data_search in lines_project_data_file_search:
        ...
        for tweettext_search in tweet_text_words:
            list_words_checked.append(tweettext_search)
            if list_words_checked.count(tweettext_search) <= 1:
