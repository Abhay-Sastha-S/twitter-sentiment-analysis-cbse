...
raw_data = pd.read_csv("C:/Users/MSI/Desktop/final final data.csv")
# print(raw_data)
...
def search_in_data_file(searchdatafile, search_word= input('enter search term --')):
    ...
    latitudes = [fltrd_df.iloc[:,5]]
    longitudes = [fltrd_df.iloc[:,6]]
    coord_df = pd.DataFrame()
    print("this cord",coord_df)
    coord_df["lat"]=longitudes
    coord_df["long"] =latitudes
    #coord_df["details"] = [fltrd_df.iloc[:,2]]
    coord_df["color coding"] = "#17BECF"
    fig = px.scatter_geo(coord_df, lat='lat', lon='long',# hover_name="details",
